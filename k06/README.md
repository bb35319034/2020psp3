...
## ソースコードの説明
 まず表をnap_value[items][nap_size] = 0で初期化し、メニュー１つずつについて、サイゼリアのメニューで価格の少ないところから順番に処理していく。処理について説明すると、メニューが入らない場合には上の値の状態を引き継ぎ、メニューが入る場合にはメニューを入れ替えない場合の価値と入れ替えた場合の価値を比較してより多いほうを入れる。出力したいものは表の一番右下の値になるのでprintfで表示されるようにする。


## 出力結果

```
PS C:\Users\admin\Desktop\2020psp3\k06> ./k06
max calorie = 1796
```

...
- LoadData: 市町村別人口タをデー読み込み、City型の配列に格納する  
- main: メイン関数。データをロードし、リニアサーチ、バイナリサーチを呼び出す  


## ソースコードの説明
　linearサーチの関数では、配列の左端と入力した値を比較して不一致だったら次の値と比較していき、一致した場合にresultにその値を入れてbreakで繰り返しを抜け出して、returnでresultの値を返している。

　binaryサーチの関数では、まずリストの中央値を出し、入力した値と一致した場合にresultにその値を入れてbreakで繰り返しを抜け出して、もし入力した値がリストの中央値よりも大きい場合には、leftを中央値の次の値にしてまた中央値を求めて、小さい場合にはrightを中央値の一つ前の値にしてまた中央値を求めてを中央値が入力した値と一致するまで繰り返している。
